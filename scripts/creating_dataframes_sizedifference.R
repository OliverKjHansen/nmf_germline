...
colnames(insertions_matrix) <- c("region","size","kmer","counts")
deletions_matrix<- read.delim(deletions_counts,  sep="\t", header = FALSE)
colnames(deletions_matrix) <- c("region","size","kmer","counts")
...
ms_ins_matrix <- pivot_wider(insertions_matrix, names_from = c(kmer, size), values_from = counts, names_glue = "{kmer}_{size}", values_fill = 0, names_sort = TRUE)
ms_ins_matrix <- ms_ins_matrix[str_order(ms_ins_matrix$region, numeric = TRUE),]

ms_del_matrix <- pivot_wider(deletions_matrix, names_from = c(kmer, size), values_from = counts, names_glue = "{kmer}_{size}", values_fill = 0, names_sort = TRUE)
...
# ms_bck_3_matrix <- ms_bck_3_matrix[str_order(ms_bck_3_matrix$Row.names, numeric = TRUE),] %>% remove_rownames %>% column_to_rownames(var="Row.names")

ms_ins_df <- list("count_matrix" = ms_ins_matrix,
             "background_matrix" = ms_bck_3_matrix)
...
ms_bck_merged_matrix <- merge(x= ms_bck_3_matrix, y = ms_bck_3_matrix, by = 0, all = TRUE, suffixes = c("_ins","_del"))
